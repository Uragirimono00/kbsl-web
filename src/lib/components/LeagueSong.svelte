    ...
    >
        <div class="bg-white space-y-3 p-4 rounded-lg shadow">
            <div class="flex items-center space-x-2 text-sm">
                ...
                <button class="p-1 text-xs text-white font-medium uppercase tracking-wider text-green-800 bg-danger-600 rounded-lg" on:click={openPopup}>
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.985V5.653z" />
                    </svg>
                </button>
                <a class="p-1 text-xs flex font-medium uppercase tracking-wider text-white bg-pink-600 rounded-lg" href="https://beatsaver.com/maps/{args.songId}" target="_blank">
                        <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 200 200" version="1.1">
                            ...
                            <path d="M 26,77 85,106 53,130 Z" stroke-linejoin="round"></path> </g>
                        </svg>
<!--                        <span class="flex items-center">{args.songId}</span>-->
                </a>
                <a class="p-1 text-xs flex font-medium uppercase tracking-wider text-white bg-blue-500 rounded-lg" href="beatsaver://{args.songId}">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" >
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9.75v6.75m0 0l-3-3m3 3l3-3m-8.25 6a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
                    </svg>
                </a>
                <a class="p-1 text-xs flex font-medium uppercase tracking-wider text-white bg-yellow-500 rounded-lg" href="{args.downloadUrl}">
                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" >
                ...
                {#if isPopupOpen}
                    <div class="overlay" on:click={closePopup}>
                        <PopupContent songId={args.songId} songDifficulty={args.songDifficulty} songModeType={args.songModeType} onClose={closePopup}/>
                    </div>
                {/if}
            </div>
